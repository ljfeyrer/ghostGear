...
original_path <- (here::here("input/"))
    ...
    # a list of sheets 
    sheets_to_read <- c("SURVEY M3", "SURVEY M4", "SURVEY M5", "SURVEY M6", "SURVEY M7", "SURVEY M8","SURVEY M9","SURVEY M10","SURVEY M11","SURVEY M12",
...
#create var for the surveys that have been annotated
    all_data = all_data%>%mutate(Debris_ID = as.factor(`Debris #`), Type = as.factor(`Litter Type (ICES)`),Reviewed = ifelse(is.na(Debris), "NO", "YES"))
    ...
    videos = all_data%>%group_by(Survey, Debris_ID, Type)%>%dplyr::summarise(Debris_unique = n( ))%>%na.omit(Debris_ID)
